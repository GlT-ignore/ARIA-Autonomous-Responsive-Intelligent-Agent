...
type InternalStep = { action: string; target?: string | undefined; value?: string | undefined; url?: string | undefined };
...
document.getElementById('btn-run-task')?.addEventListener('click', async () => {
        ...
        try {
            ...
            if (classified.type === 'data_extraction') {
                try {
                    ...
                    const resultsContainer = document.getElementById('results-container');
                    if (resultsContainer) {
                        const targets: ExtractionTarget[] = []; // Would need to pass from workflow
    ...
    if (cfg.mode === 'llm' && agentCfg.enabled) {
        ...
        const execOne = async (s: InternalStep, i: number, total: number) => {
            ...
            for (let i = 0; i < maxIterations; i++) {
                ...
                if (i === 0 && plannedStartUrl && plannedStartUrl.startsWith('http')) {
                    ...
                    const prompt = createNextActionPrompt({
                        snapshot: optimized,
                        task: t,
                        recentSteps: recent.map(r => {
                            const step: { action: string; target?: string; value?: string; url?: string; ok?: boolean; error?: string } = {
                                action: r.action
                            };
                            if (r.target !== undefined) step.target = r.target;
                            if (r.value !== undefined) step.value = r.value;
                            if (r.url !== undefined) step.url = r.url;
                            if (r.ok !== undefined) step.ok = r.ok;
                    ...
                    try {
                        ...
                        log({ error: 'LLM call failed', details: String(e) });
                    }
                ...
                if (!step) {
                    ...
                    // No need to use text here if it failed, but if it exists it will be used
                    log({ warning: 'LLM returned no parseable action', firstLine: (step as any)?.firstLine || 'none' });
                    continue;
                }
                ...
                if (step.action === 'DONE') {
                    ...
                    if (isVideoPlayback && hasTypedSearch && !hasClickedAfterSearch) {
                        ...
                        const nextStep: InternalStep & { ok?: boolean; error?: string } = { action: 'WAIT', value: '1500', error: 'premature-done-rejection' };
                        step = nextStep;
                    ...
                    if (isVideoPlayback) {
                        ...
                        if (!currentUrl.includes('/watch')) {
                            ...
                            const nextStep: InternalStep & { ok?: boolean; error?: string } = { action: 'WAIT', value: '1000', error: 'premature-done-rejection' };
                            step = nextStep;
                            recent.push({ ...step, ok: true });
                            await new Promise(r => setTimeout(r, 1000));
                            continue;
                ...
                for (let retry = 0; retry < maxRetries && !succeeded; retry++) {
                    try {
                        ...
                        await execOne(step, i, maxIterations);
                        succeeded = true;
                        recent.push({ ...step, ok: true });
